...
import (
	...
	armhelpers "github.com/Azure/acs-engine/pkg/armhelpers"
...
func (c *Cluster) getARMClient() error {
	...
	if client, err = armhelpers.NewAzureClientWithClientSecret(env,
		c.subscriptionId,
		c.clientId,
...
func (c Cluster) createCluster() error {
	...
	err = c.getARMClient()
	if err != nil {
		fmt.Errorf("Failed to get ARM  Client %v", err)
	}
	err = c.getTemplates()
	if err != nil {
		fmt.Errorf("Failed to get templates %v", err)
	}
